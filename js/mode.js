(function(){
	$(function(){
		...
		var count=2,num=1,index=null,shoppingnum=1;
		/*导航栏的开始*/
		...
		$('.small_pic ul li').on('mouseenter',function(){
			...
			$('.img-big li').eq(index).css('display',"block");
		});
		$('.move_left').on('click',function(){
			if(num>$('.mark ul li').length-4) return;
		...
		$('#span2').on('mousemove',function(e){
			...
			if (l<0) {
				...
				l=$('#div1').width()-$('#span1').width();
			}
			if(t<0){
				t=0;
			}else if(t>$('#div1').height()-$('#span1').height()){
				t=$('#div1').height()-$('#span1').height();
			}
			var x=l/($('#div1').width()-$('#span1').width());
			var y=t/($('#div1').height()-$('#span1').height());
			$('#span1').css('left',l);
			$('#span1').css('top',t);
